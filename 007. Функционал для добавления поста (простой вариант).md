...
1. Создать файл «templates/store/product_add.html» и записать в него код:  
```html 
{% extends "base.html" %}
...
3. Внести изменения в файл «store/urls.py»:  
	\- после строки «`path("products/<int:product_id>/", views.get_post_detail, name='product_detail'),`» добавить строку:  
```python 
path('products/add/', views.create_product),
``` 

То, что новые товары создались, можно посмотреть либо в админке сайта в разделе «Товары», либо в БД «db.sqlite3» в таблице «products».

4. (Опционально.) Создать коммит: «feat(posts): Добавил функционал для добавления поста (самый простой вариант)».
