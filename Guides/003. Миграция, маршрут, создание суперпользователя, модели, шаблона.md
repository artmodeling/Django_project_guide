...
20. Создание маршрута:  
    ...
    2) Для начала можно в них внести эти коды, следить за форматированием, нужны отступы слева и пустые строки между строкам, как указано ниже, комментарии в самом коде можно не указывать:  
        ...
        # Здесь уже нужно создать первый шаблон. Путь 'store/product_list.html' здесь не нужно указывать полный. 
...
24. В данном случае нужно выполнить следующие действия:  
    ...
    5) И уже в папке «`store`», которая лежит в папке «`templates`», создать файл «`product_list.html`» - в нем будет создан первый шаблон страницы HTML.  
    <picture>
...
В файле «`store\templates\store\product_list.html`» можно написать, например, такой код, следить за форматированием, нужны отступы слева и пустые строки между строкам, как указано ниже:
```html
<!DOCTYPE html>
<html>
<head>
...
31. Проверить на сайте, что все работает.

32. (Опционально.) Создать и запушить коммит. Текст его может быть, например, таким:  
"""  
